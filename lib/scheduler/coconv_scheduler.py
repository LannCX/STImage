...
class CoConvScheduler(BaseScheduler):
    def __init__(self, net, cfg, logger=None):
        super(CoConvScheduler, self).__init__(net, cfg, logger=logger)
    ...
    def init_op(self):
        ...
        if self.cfg.PRINT_PARAM:
            # dump_input = torch.rand(1, 3, self.cfg.DATASET.CROP_SIZE[0]*self.cfg.DATASET.T_SIZE,
            #                         self.cfg.DATASET.T_SIZE*self.cfg.DATASET.CROP_SIZE[0])
            dump_input = torch.rand(1, self.cfg.DATASET.T_SIZE**2, 3, self.cfg.DATASET.CROP_SIZE[0] ,
                                    self.cfg.DATASET.CROP_SIZE[0])
    ...
    def feed_data_and_run_loss_op(self, data):
        ...
        outputs = self._net(input_tensor)

        loss = self.cls_criterion(outputs, labels)
        acc = accuracy(outputs.data, labels, topk=(1,))[0]
    ...
    def predict_in_tst(self, tst_loader):
        ...
        for data in tst_loader:
            ...
            input_tensor = input_tensor.reshape((-1,)+input_tensor.shape[-3:]).to(self.device)
            labels = labels.to(self.device)
            output = self._net(input_tensor)
            # pred = F.softmax(output, dim=1).mean(dim=0)
            prec1, prec5 = accuracy(output.data, labels, topk=(1, 5))
            top1.update(prec1.item(), input_tensor.size(0))
            top5.update(prec5.item(), input_tensor.size(0))
        nbar.close()
        cnt_time = time.time()-start_time
        self.logger.log('=>Evaluation is finished, \n '
                        '=>Average {:.3f} sec/video \n'
                        '=>Prec@1 {:.02f}% Prec@5 {:.02f}%'.format(float(cnt_time)/len(tst_loader), top1.avg, top5.avg))
