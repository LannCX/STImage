...
class Res2Net(nn.Module):
    ...
    def forward(self, x):
        ...
        x = x.view(x.size(0), -1)
        x = self.fc(x)
        x = x.view((-1, 16)+x.size()[1:]).mean(dim=1)
...
def tea50_16f(cfg):
    ...
    if cfg.MODEL.PRETRAINED:
        static_dict = torch.load(cfg.MODEL.PRETRAINED)
        model.load_org_weights(static_dict)
    return model
...
def parse_args():
    ...
    parser.add_argument('--cfg', default='../../experiments/tea-something-something.yaml',
