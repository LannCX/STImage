        ...
        if os.path.exists(save_path):
            ...
            for i in range(df.shape[0]):
                ...
                if split == 'train':
                    ...
                    vid_path = os.path.join(data_root, split, label, vid_id + '.mp4')
                if os.path.exists(vid_path):
                    test_data = fast_read_sel_frames_from_vid_file(vid_path, [0])
                    if len(test_data)==0:
                        print(vid_path)
                        continue
                    self.anno.append({'id': vid_id, 'path': vid_path, 'label': label.replace('_', ' '), 'is_cc': is_cc})
